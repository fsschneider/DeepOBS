...
def _inception_v3(x, training, l2_reg):
    def conv2d_BN(inputs, filters, kernel_size, strides, padding, training):
        ...
        x = tf.compat.v1.layers.batch_normalization(x, momentum=0.9997, training=training)
        x = tf.nn.relu(x)
        return x
    ...
    def inception_block5(inputs, variant, training, name="inception_block5"):
        ...
        with tf.compat.v1.variable_scope(name):
            with tf.compat.v1.variable_scope("branch0"):
                branch0_1x1 = conv2d_BN(inputs, 64, 1, (1, 1), "SAME", training)
            with tf.compat.v1.variable_scope("branch1"):
                branch1_pool = tf.compat.v1.layers.average_pooling2d(inputs, 3, (1, 1), "SAME")
                branch1_1x1 = conv2d_BN(
                    branch1_pool, num_filters, 1, (1, 1), "SAME", training
                )
            with tf.compat.v1.variable_scope("branch2"):
                branch2_1x1 = conv2d_BN(inputs, 48, 1, (1, 1), "SAME", training)
                branch2_5x5 = conv2d_BN(branch2_1x1, 64, 5, (1, 1), "SAME", training)
            with tf.compat.v1.variable_scope("branch3"):
    ...
    def inception_block10(inputs, training, name="inception_block10"):
        ...
        with tf.compat.v1.variable_scope(name):
            with tf.compat.v1.variable_scope("branch0"):
                branch0_pool = tf.compat.v1.layers.max_pooling2d(inputs, 3, (2, 2), "VALID")
            with tf.compat.v1.variable_scope("branch1"):
    ...
    def inception_block7(inputs, training, name="inception_block7"):
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope("branch3"):
                branch3_pool = tf.compat.v1.layers.average_pooling2d(inputs, 3, (1, 1), "SAME")
    ...
    with tf.compat.v1.variable_scope("stem"):
        ...
        x = tf.nn.max_pool2d(input=x, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="VALID")
