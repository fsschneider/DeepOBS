# -*- coding: utf-8 -*-
"""
VGG 19 architecture adapted for Cifar-10. The input image will be padded to fit the 224x224 size of the original VGG network and 10 output classes are used.
"""
...
class set_up:
    """Class providing the functionality for the VGG 19 architecture on `CIFAR-10`. The `CIFAR-10` images are resized to ``224`` by ``224`` to fit the input dimension of the original VGG network, which was designed for `ImageNet`.

    Details about the architecture can be found in the `original paper`_. VGG 19 consists of 19 weight layers, of mostly convolutions. The model uses cross-entroy loss. A weight decay is used on the weights (but not the biases) which defaults to ``5e-4``.
