...
class cifar10_vgg16(TestProblem):
  ...
  Args:
    ...
    l2_reg (float): L2-regularization factor. L2-Regularization (weight decay)
        is used on the weights buloss_function: The loss function for this testproblem is torch.nn.CrossEntropyLoss()
    net: The DeepOBS subclass of torch.nn.Module that is trained for this tesproblem (net_vgg).t not the biases.
        Defaults to ``5e-4``.
    ...
    def __init__(self, batch_size, l2_reg=0.0005):
        ...
        Args:
          ...
          l2_reg (float): L2-regularization factor. L2-Regularization (weight decay)
              ...
              Defaults to ``5e-4``.
        """
        super(cifar10_vgg16, self).__init__(batch_size, l2_reg)
