...
class cifar100_wrn404(TestProblem):
    r"""DeepOBS test problem for the Wide ResNet 40-4 architecture for Cifar-100.

    Details about the architecture can be found in the `original paper`_.
    L2-Regularization is used on the weights (but not the biases) which defaults
    to ``5e-4``.
    ...
    def get_regularization_groups(self):
        ...
        Returns:
            dict: A dictionary where the key is the regularization factor and
                the value is a list of parameters.
        """
