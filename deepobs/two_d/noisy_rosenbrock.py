...
class set_up:
    """Simple 2D Noisy Beale Loss Function:
        ...
        :math:`(1 - u)^2 + 100 \cdot x \cdot (v - u^2)^2`
    ...
    def __init__(self, batch_size=128, size=1000, noise_level=3, starting_point=[-0.5, 1.5], weight_decay=None):
        ...
        Args:
            batch_size (int): Batch size of the data points. Defaults to ``128``.
            size (int): Size of the training set. Defaults to ``1000``.
            noise_level (float): Noise level of the training set. All training points are sampled from a gaussian distribution with the noise level as the standard deviation. Defaults to ``3``.
            starting_point (list): Coordinates of the starting point of the optimization process. Defaults to ``[-0.5, 1.5]``.
            weight_decay (float): Weight decay factor. In this model there is no weight decay implemented. Defaults to ``None``.
    ...
    def set_up(self, starting_point=[-0.5, 1.5], weight_decay=None):
    ...
    def rosenbrock(self, u, v):
        ...
        Returns:
            float: Function value of the deterministic Rosenbrock function at ``(u,v)``.
