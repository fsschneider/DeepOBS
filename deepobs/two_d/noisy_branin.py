...
class set_up:
    ...
    def plot_run(self, u_history, v_history, loss_history):
        """Plot the history of weights (u, v) and the corresponding loss of a optimizer. Plot the Deterministic Branin as well."""
        # meshgrid
    ...
    def anim_run(self, u_history, v_history, loss_history, name="Optimizer Trajectory"):
        """Animate the history of weights (u, v) and the corresponding loss of a optimizer. Plot the Deterministic Branin as well."""
        # meshgrid
        traj_box = [[min(u_history), max(u_history)], [
            min(v_history), max(v_history)]]
        plot_limits = [[min(-5.0, traj_box[0][0]), max(10, traj_box[0][1])],
