...
if [ "$prepare_svhn" = "true" ]
  ...
  python -c "import deepobs; deepobs.svhn.svhn_preprocess.preprocess(file_path='$DATA_FOLDER/svhn/', save_path='$DATA_FOLDER/svhn/')"
...
if [ "$prepare_tolstoi" = "true" ]
  ...
  python -c "import deepobs; deepobs.tolstoi.tolstoi_preprocess.preprocess(file_path='$DATA_FOLDER/tolstoi/', save_path='$DATA_FOLDER/tolstoi/', encoding='utf-8', num_examples=0, test_size=0.2)"
