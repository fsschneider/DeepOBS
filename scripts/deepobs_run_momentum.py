...
# -*- coding: utf-8 -*-
"""
Run MomentumOptimizer on a deepobs test problem.

Runs tensorflow's built-in momentum optimizer on a test problem from the deepobs package.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run SGD with Momentum on a DeepOBS test problem.")
    ...
    parser.add_argument("--nesterov", action="store_const",
                        const=True, default=False,
                        help="Add this flag to use Nesterov's variant of Momentum.")
...
def main(test_problem, bs, lr, mu, N, nesterov=False, data_dir=None, wd=None, lr_sched_epochs=None, lr_sched_factors=None, checkpoint_epochs=1, random_seed=42, nologs=False, train_log_interval=10, print_train_iter=False, saveto="results", pickle=False, no_time=False, run_name=""):
    ...
    name += "__mu_" + deepobs.run_utils.float2str(args.mu)
    ...
    opt = tf.train.MomentumOptimizer(lr, momentum=args.mu,
    ...
    if not args.nologs:
        ...
        momentum_summary = tf.summary.scalar("hyperparams/momentum", args.mu, collections=[tf.GraphKeys.SUMMARIES, "per_epoch"])
